...
Failure analysis found MATH evaluator false negatives caused by text-answer formatting such as \texttt{MAKE} versus \texttt{\textbackslash text\{MAKE\}}, comma spacing inside text answers, and reference solutions that express multi-answer outputs as multiple \texttt{\textbackslash boxed\{\}} values. The evaluator now normalizes simple text wrappers, compares comma-separated answer lists as unordered lists, and combines multiple boxed reference answers before symbolic comparison. Remaining MATH baseline failures include genuine reasoning/counting issues, such as the rectangle-counting problem solved by \texttt{manual\_v1} but missed by CoT.
...
\begin{enumerate}
    \item Run 30--50 validation examples for direct, CoT, and \texttt{manual\_v1} to check whether the 20-sample MATH gain persists.
