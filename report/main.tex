...
Table~\ref{tab:smoke} reports one-sample smoke tests. These are not final benchmark results; they only prove that the API, dataset loader, scoring logic, and workflows are connected correctly.

\begin{table}[t]
\caption{One-sample smoke tests on validation splits.}
\label{tab:smoke}
\vskip 0.08in
\begin{center}
\begin{small}
...
The next experimental stage is to expand from one-sample smoke tests to controlled subsets and then full benchmark runs. The planned sequence is:
...
Current results are only smoke tests and should not be interpreted as performance gains. The manual workflows increase token usage and latency, especially for MATH where three candidates plus ensemble selection can take around two minutes for one problem. Kimi pricing is not yet reflected in the local cost field because the imported AFlow pricing table does not include Kimi. Future runs should report raw token counts or add a verified Kimi pricing table.

\section{Conclusion}

The project now has a reproducible AFlow-based implementation path: safe Kimi configuration, dataset download, direct and CoT baselines, manual multi-agent workflows, smoke-test result tracking, and a report draft. The remaining core work is to scale experiments to meaningful sample sizes and use validation failures to improve the workflows before final test evaluation.
