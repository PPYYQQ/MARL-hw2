...
- Report coverage includes research context, environment installation, baseline prompts, proposed workflow design, algorithmic rationale, validation/test results, ablations, failure analysis, efficiency analysis, observations/reflections, limitations, and optional follow-up work.
...
Package verification checks tracked entries, clean tracked worktree state, optional PDF inclusion, manifest commit, manifest output path, manifest file counts, per-file SHA-256 checksums against both the archive and current filesystem, the embedded zero-failure audit summary, forbidden secret/run paths, and the active `KIMI_API_KEY` value when it is set.
