...
| `make verify-package` | Verifies the zip against tracked files, clean tracked worktree state, current commit, included PDF, manifest file counts, archive/filesystem checksums, zero-failure audit summary, forbidden secret/run paths, and active `KIMI_API_KEY` leakage when available |
