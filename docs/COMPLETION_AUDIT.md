...
- Final metadata commands reject placeholder/example metadata for normal finalization and real-metadata dry-runs; `make finalize-dry-run` is the only maintained path that intentionally enables `--allow-test-metadata`.
